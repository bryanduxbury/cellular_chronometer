...
require File.expand_path(File.dirname(__FILE__) + "/../hierarchical_duparc_ataviser")
require File.expand_path(File.dirname(__FILE__) + "/../intersecting_row_ataviser")

describe HierarchicalDuparcAtaviser do
  ...
  #   g.set(0,0)
  # 
  #   da = HierarchicalDuparcAtaviser.new(ataviser)
  # 
  ...
  it "should find at least one known prior generation" do
    ...
    priors = HierarchicalDuparcAtaviser.new(ataviser).prior_generations(g)
    # puts priors.size
  ...
  it "should behave the same as the regular DuparcAtaviser"
end